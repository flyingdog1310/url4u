...
function renderClickCompare(data, clickNowDisplay, clickBeforeDisplay, clickCompareDisplayI, clickCompareDisplayColor) {
    ...
    if (clickNow > clickBefore) {
        ...
        clickBeforeDisplayId.textContent = `${Math.floor((clickNow / clickBefore) * 100)}%`;
    }
    if (clickNow < clickBefore) {
...
function renderTopClick(data) {
    ...
    const displayNameGroup = [displayName1, displayName2, displayName3, displayName4, displayName5];

    for (let i = 0; i < data.length; i++) {
        displayNameGroup[i].innerHTML = `${data[i].time_range.slice(0, 16).replace("T", " ")}
...
function getDeviceClick() {
    ...
    fetch(`/api/1.0/device_click/${window.location.pathname.split("/")[3]}`, {
        ...
        .then((data) => {
            ...
            renderTopSource(data, ["device1", "device2", "device3", "device4", "device5"]);
        })
        .catch((error) => {
            console.log(error);
        });
...
function getReferrerClick() {
    ...
    fetch(`/api/1.0/referrer_click/${window.location.pathname.split("/")[3]}`, {
        ...
        .then((data) => {
            ...
            renderTopSource(data, ["referrer1", "referrer2", "referrer3", "referrer4", "referrer5"]);
        })
        .catch((error) => {
            console.log(error);
        });
